...
void inserir( struct Lista * l , int chavenum, int chaveden){
    ...
    while(i < l->quantidade && !achei){
        if((l->numerador[i] / (l->denominador[i]*1.0)) == (chavenum/(chaveden*1.0)) ){
            return;
        }else if(chavenum/(chaveden*1.0) < l->numerador[i] / (l->denominador[i]*1.0)){
            achei = 1;
        }else{
            i++;
        }
...
int buscaBinaria(int * v, int * k, int e, int d, double x){
    int meio;
    while(e <= d){
        meio = (e + d)/2;
        if((v[meio]/(k[meio]*1.0)) == x){
            return meio;
        }
        else if ((v[meio]/(k[meio]*1.0)) < x){
            e = meio+1;
        }
        else{
            d = meio-1;
...
void remover(struct Lista * l, double chave){
