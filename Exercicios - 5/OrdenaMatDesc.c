...
void OrdenaMatDesc(struct Lista *);
...
void OrdenaMatDesc(struct Lista * l){
    int i, w, j, temp;
    ...
    for(j = 0; j < l->tamanhocoluna; j++){
     ...
     for(j = 0; j < l->tamanholinha; j++){
        for(i = l->tamanhocoluna-1; i > 0; i--){
            for(w = 0; w < i; w++){
                if(l->matriz[j][w+1] > l->matriz[j][w]){
                    temp = l->matriz[j][w];
                    l->matriz[j][w] = l->matriz[j][w+1];
                    l->matriz[j][w+1] = temp;
                }
            }
        }
